import getContent from "./../../scripts/aboutDistrict.js";
...
document.addEventListener('DOMContentLoaded', ()=>{
    (async function getHeader(){
        ...
        try {
            ...
            let runFunction = sessionStorage.getItem('runFunction');
            ...
            allDistrictSubmenu.forEach(item =>{
                // item.setAttribute('href', 'aboutDistrict.html');
                item.onclick = (e)=>{
                    e.preventDefault();
                    try {
                        let activeContent = Object.keys(menuDistrict).find(key => menuDistrict[key] == e.target.innerHTML); 
                        console.log(activeContent);
                        sessionStorage.setItem('runFunction', activeContent);
                        window.location.href = 'aboutDistrict.html'
                    } catch (error) {
                        console.log(error);
                    }
            ...
            if(runFunction){
                getContent(sessionStorage.getItem(runFunction));
                sessionStorage.removeItem('runFunction')
            }
