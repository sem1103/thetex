let mobMenu,subMobMenuBtn, descMenu,subMenu, lines, subMenuId,allDistrictSubmenu, allPressCentreSubmenu, allGrovernmentSubmenu, allDocumentsSubmenu;
let menuDistrict = {
...
let documentsMenu = {
    searchDocuments: 'Поиск по документами',
    regulatoryActs: 'Нормативно-правовые акты',
    regulationsMunicipalServices: 'Регламенты государственных и муниципальных услуг',
    reports: 'Отчеты',
    antiCorruptionExpertise: 'Независимая антикоррупционная экспертиза нормативно-правовых актов'
...
document.addEventListener('DOMContentLoaded', ()=>{
    (async function getHeader(){
        ...
        try {
            ...
            allDocumentsSubmenu.forEach(item =>{
                item.onclick = (e)=>{
                    let activeContent = Object.keys(documentsMenu).find(key => documentsMenu[key] == e.target.innerHTML); 
                    window.location.href = `documents.html?${activeContent}`;
                }
            });
...
function initMenu(){
    ...
    allDocumentsSubmenu = document.querySelectorAll('.documents a');
...
export {menuDistrict, menuPressCentre, menuGovernment, publicReceptionMenu, documentsMenu};