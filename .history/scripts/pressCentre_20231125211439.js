...
console.log(menuPressCentre);

if(location == 'news' || location == '') showNewsItems();
Object.keys(menuPressCentre).forEach((key, index) => {
    if(key == location) {
        sideMenu[index].classList.add('active__menu');
    }else if(location == '') {
        sideMenu[0].classList.add('active__menu')
    }else sideMenu[index].classList = '';
