...
function getAllDocuments(pattern, content){
    ...
    allDocs.forEach(item => {
        ...
        item.text == undefined && (text.innerText = item.text);
        downloadBtn.onclick = ()=> downloadDoc(item.link)
        pattern.onclick = ()=> downloadDoc(item.link);


        content.appendChild(pattern.cloneNode(true))
