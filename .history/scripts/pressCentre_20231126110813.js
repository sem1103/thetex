...
async function showContent(fileName ){
    ...
    if(index <= arr.length){
        arr.slice(index, index + 3).forEach(item =>{
            ...
            clonePattern.onclick = ()=> playVideo(item.iframeSrc, item.iframeTitle);
...
document.addEventListener('DOMContentLoaded', ()=>{
    console.log(document.querySelector('.videoGallery'));
})

async function playVideo(src, title){
    let modal = document.querySelector('.video__modal');
    let iframe = document.querySelector('.video__modal iframe');
    iframe.setAttribute('src', src);
    iframe.setAttribute('title', title);
    iframe.addEventListener('load', function onLoad() {
        // Убираем обработчик события, чтобы не вызывать его повторно
        iframe.removeEventListener('load', onLoad);

        // Отображаем модальное окно
        modal.classList.add('video__modal__show');
        setHeigthIframe()
    });
