...
let eventsArray = [
    ...
    },
    {
        ...
        newsSource: 'Раздел',
        title: 'Длинный заголовок в несколько строк',
        text: 'Текст — зафиксированная на каком-либо материальном носителе человеческая мысль; в общем плане связная и полная последовательность символов.\n\nСуществуют две основные трактовки понятия «текст»: имманентная и репрезентативная.Существуют две основные трактовки понятия «текст»: имманентная и репрезентативная. Текст — зафиксированная на каком-либо материальном носителе человеческая мысль; в общем плане связная и последовательность символов. ',
        data: '14 февраля 2023'
    }
]
let index = 0;
...
async function showContent(fileName, showFlag = true ){
    ...
    let newsSource = pattern.querySelector('.source');
    let titleLink = pattern.querySelector('a');
    let newsText = pattern.querySelector('.text');
    let newsData = pattern.querySelector('.data');
    let addBtn = document.querySelector('.content__container .addItemBtn');
    let array = [];

    if(fileName == 'news') array = newsArray;
    else if(fileName == 'events') array = eventsArray;
    
    if(index <= array.length && showFlag){
        array.slice(index, index + 3).forEach(item =>{
            img.src = item.img;
...
else if(location == 'events') showContent('events')
