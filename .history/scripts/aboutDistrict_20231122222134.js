...
menuBtns.forEach(item =>{
    ...
    item.onclick = elem => getContent(elem.target.innerText,fileName)
});


async function getContent(elemText,fileName){
    let divTemp = document.createElement('div');
    menuBtns.forEach(item => {
        if(item.innerHTML == menuArray[fileName]) item.classList.add('active__btn')
