...
async function showContent(fileName ){
    ...
    if(index <= arr.length){
        arr.slice(index, index + 3).forEach(item =>{
            ...
            fileName == 'photoGallery' && (clonePattern.onclick = ()=> showPhotoSliter(item.folderName, item.quantity))
...
/*----PhotoGallery----*/
function showPhotoSliter(gallery, photoQuantity) {
    ...
    contentWrapper.classList.toggle('hide__slider')
}

function closeSlider(){
   document.querySelector('.content__wrapper').classList.toggle('hide__slider');
