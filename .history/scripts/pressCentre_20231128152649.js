...
let newsArray = [
    {
        ...
        link: 'resultofconferencenews.html'
    },
    {
...
async function showContent(fileName ){
    ...
    if(index <= arr.length){
        arr.slice(index, index + 3).forEach(item =>{
            ...
            let title = clonePattern.querySelector('h3');
            ...
            !titleLink && (title.innerText = item.title);
...
function showPhotoSliter(gallery, photoQuantity, title) {
    ...
    let link = 'pressCentre.html?photoGallery'
    ...
    addBreadCrunps({
        ...breadCrumps,
        ['?'+location]: menuPressCentre[location],
        as: title
