...
async function getNewsItem(){
    ...
    let elem = divTemp.querySelector('.news__item')
    return elem
    console.log(divTemp.querySelector('.news__item'));
}


function showNewsItems(quantity){
    let content = document.querySelector('.content');
    let wrapper = document.createElement('div');
    let pattern = getNewsItem();
    wrapper.classList.add('news__wrapper');

    for(let i = 0; i < quantity; i++){
        wrapper.innerHTML += pattern.innerHTML
    }
    console.log(pattern);
