...
async function showNewsItems(){
    ...
    if(index <= newsArray.length){
        newsArray.slice(index, index + 3).forEach(item =>{
            let clone = pattern.cloneNode(true);
            let imgClone = clone.querySelector('img');
            let newsSourceClone = clone.querySelector('.news__source');
            let titleLinkClone = clone.querySelector('a');
            let newsTextClone = clone.querySelector('.news__text');
            let newsDataClone = clone.querySelector('.data');

            imgClone.src = item.img;
            imgClone.setAttribute('data-src', item.img);
            newsSourceClone.innerText = item.newsSource;
            titleLinkClone.innerText = item.title;
            titleLinkClone.href = 'pressCentre.html';
            newsTextClone.innerText = item.text;
            newsDataClone.innerText = item.data;
